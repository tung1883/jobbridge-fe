...
export function MyCVs() {
  ...
  async function handleUpload(e) {
    ...
    if (!["pdf", "docx"].includes(ext)) { setUploadErr("Only PDF and DOCX files are accepted."); return; }
    setUploading(true); setUploadErr("");
    try { await cvs.upload(file); refetch(); }
  ...
  async function handleDelete(id) {
    ...
    try { await cvs.delete(id); refetch(); }
      ...
      <div className="page-header-row">
        ...
        <div>
          ...
          <button type="button" className="btn btn-primary" onClick={() => fileRef.current?.click()} disabled={uploading}>
            {uploading ? <><Spinner size="sm" white />Uploading…</> : "+ Upload CV"}
          </button>
        </div>
      </div>

      {uploadErr && <Alert type="danger" onClose={() => setUploadErr("")}>{uploadErr}</Alert>}
      {loading && <LoadingPage />}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          {list.map((cv) => (
            <div key={cv.id} className="card-flat" style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
              <div style={{ width: 44, height: 44, background: "var(--cream)", border: "1px solid var(--border)", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.25rem", flexShrink: 0 }}>
                📄
              </div>
              ...
              <div style={{ display: "flex", gap: "0.5rem", flexShrink: 0 }}>
                {cv.url && (
                  <a className="btn btn-secondary btn-sm" href={cv.url} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()}>
                    Preview
                  </a>
                )}
                <button
