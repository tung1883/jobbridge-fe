  ...
  if (saved.length === 0) {
    return (
      <div className="page-enter">
        <div className="page-header"><div className="page-title">Saved Jobs</div></div>
        {error && <Alert type="danger">{error}</Alert>}
        <EmptyState
          icon="🔖"
          title="No saved jobs"
          desc="Bookmark jobs while browsing to keep track of them here."
          action={<button type="button" className="btn btn-primary" onClick={() => navigateTo("jobs")}>Browse jobs</button>}
        />
      </div>
    );
  }
  ...
  return (
    <div className="page-enter">
      <div className="page-header">
        ...
        <div className="page-sub">{saved.length} job{saved.length !== 1 ? "s" : ""} bookmarked</div>
      </div>
      ...
      {selected && (
        <JobDetailModal job={selected} onClose={() => setSelected(null)} user={user} />
      )}
    </div>
  );
}
