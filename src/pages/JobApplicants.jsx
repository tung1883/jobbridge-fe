...
import {
  Alert, Spinner, StatusBadge, EmptyState, LoadingPage, SalaryDisplay, DateDisplay,
} from "../shared.jsx";
...
export function JobApplicants({ job, onBack }) {
  ...
  async function handleStatusChange(appId, status) {
    setUpdatingId(appId);
    try { await applications.updateStatus(appId, status); refetch(); }
    catch { /* optional */ }
    ...
    <div className="page-enter">
      <div className="page-header-row">
        <div>
          ...
          <div className="page-sub">Reviewing for: <strong>{job.title}</strong></div>
        </div>
        <button type="button" className="btn btn-ghost" onClick={onBack}>← Back to postings</button>
      </div>
      ...
      <div className="card-flat" style={{ marginBottom: "1.5rem", display: "flex", gap: "1.5rem", flexWrap: "wrap" }}>
        {[
          ...
          ["Salary", <SalaryDisplay key="s" min={job.salary_min ?? job.min_salary} max={job.salary_max ?? job.max_salary} currency={job.currency} />],
          ["Applicants", loading ? "…" : list.length],
        ].map(([k, v]) => (
          <div key={k}>
            <div style={{ fontSize: "var(--text-xs)", color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.08em" }}>{k}</div>
      ...
      {!loading && list.length === 0 && (
        <EmptyState icon="👥" title="No applicants yet" desc="Share your posting to attract candidates." />
      )}

      {!loading && list.length > 0 && (
        <>
          {rankLoading && (
            <div className="card-flat" style={{ marginBottom: "1rem", display: "flex", alignItems: "center", gap: "0.75rem", background: "var(--accent-soft)", border: "1px solid var(--accent-muted)" }}>
              <Spinner size="sm" gold />
              <span style={{ fontSize: "var(--text-sm)", fontWeight: 500 }}>AI is ranking candidates…</span>
            </div>
          )}
          {rankData && (
            <div className="card-flat" style={{ marginBottom: "1rem", display: "flex", alignItems: "center", gap: "0.5rem", background: "var(--success-bg)", border: "1px solid rgba(21,128,61,0.2)" }}>
              <span>🤖</span>
                ...
                <tbody>
                  {list.map((app) => (
                    <tr key={app.id}>
                        ...
                        {app.aiSkills?.length > 0 && (
                          <div style={{ display: "flex", gap: "0.3rem", flexWrap: "wrap", marginTop: "0.3rem" }}>
                            {app.aiSkills.slice(0, 4).map((s) => (
                              <span key={s} className="badge badge-ink" style={{ fontSize: "0.65rem" }}>{s}</span>
                            ))}
                            {app.aiSkills.length > 4 && <span style={{ fontSize: "0.65rem", color: "var(--muted)" }}>+{app.aiSkills.length - 4}</span>}
                          </div>
                        )}
                      </td>
                      <td style={{ color: "var(--muted)" }}><DateDisplay date={app.created_at} /></td>
                      <td>
                        {app.cv_url ? (
                          <a className="btn btn-secondary btn-sm" href={fileUrl(app.cv_url)} target="_blank" rel="noreferrer">
                            View CV
                          </a>
                        ) : (
                      ...
                      <td>
                        {app.aiScore !== null ? (
                          <span style={{ fontWeight: 700, fontSize: "var(--text-md)", color: app.aiScore > 1.5 ? "var(--success)" : app.aiScore > 0.5 ? "var(--warn)" : "var(--muted)" }}>
                      ...
                      <td style={{ textAlign: "left" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
                          <select
                            ...
                            style={{ padding: "0.3rem 1.8rem 0.3rem 0.5rem", fontSize: "var(--text-sm)", width: "100%", maxWidth: 160 }}
                            value={app.status}
