...
export function JobDetailModal({ job, onClose, user, onUpdate }) {
    ...
    useEffect(() => {
        const activeRef = view === "company" ? companyPanelRef : jobPanelRef
        requestAnimationFrame(() => {
            const el = activeRef.current
            if (!el) return
            setPanelHeight(el.scrollHeight + "px")
        })
    }, [view, companyInfo, jobInfo])
        ...
        >
            ...
            >
                ...
                >
                    <div
                        ref={jobPanelRef}
                        style={{ width: "50%", minWidth: 0, boxSizing: "border-box", wordBreak: "break-word", overflowWrap: "break-word" }}
                    >
                            ...
                            >
                                ...
                                >
                                    {companyInfo.logo_url ? (
                                        <img
                                            src={fileUrl(companyInfo.logo_url)}
                                            alt=""
                                ...
                                <div style={{ minWidth: 0, flex: 1 }}>
                                    ...
                                    {companyInfo.location && (
                                        <div style={{ fontSize: "var(--text-xs)", color: "var(--muted)", marginTop: "0.2rem" }}>
                                            📍 {companyInfo.location}
                                        </div>
                                    )}
                        ...
                        {!user ? (
                            ...
                            <>
                                ...
                                {!jobInfo?.submitted?.cv_id && (
                                    <Alert>
                                        The previous CV was removed, recruiters can still see the CV but at best you should choose a new one.
                                    </Alert>
                                )}
                                    ...
                                    {!deleting ? (
                                        <>
                                            ...
                                            <button
                                                ...
                                                disabled={
                                                    loading ||
                                                    (selectedCv ?? originalCvId) === originalCvId ||
                                                    selectedCv === REMOVED_CV ||
                                                    !selectedCv
                                                }
                                            >
                                                {loading ? (
                        ...
                        {companyInfo && (
                            <>
                                ...
                                >
                                    ...
                                    <div>
                                        <div
                                            style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap", marginBottom: "0.3rem" }}
                                        >
                                        ...
                                        <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
                                            {companyInfo.industry && (
                                                <span style={{ fontSize: "var(--text-xs)", color: "var(--muted)" }}>🏭 {companyInfo.industry}</span>
                                            )}
                                            {companyInfo.location && (
                                                <span style={{ fontSize: "var(--text-xs)", color: "var(--muted)" }}>📍 {companyInfo.location}</span>
                                            )}
                                            {memberSince && (
                                                <span style={{ fontSize: "var(--text-xs)", color: "var(--muted)" }}>🗓 Since {memberSince}</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
...
function Section({ title, children }) {
    return (
        <div style={{ marginBottom: "1.25rem" }}>
            ...
            <div style={{ fontSize: "var(--text-base)", lineHeight: 1.75, color: "var(--muted)", whiteSpace: "pre-line", maxHeight: "150px", overflowY: "auto" }}>
                {children}
            </div>
        </div>
    )
